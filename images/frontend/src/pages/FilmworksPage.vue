<template>
    <div class="filmworks-page">
        ...
        <div v-if="isSearchMode && filmworks.length > 0" class="search-info">
            ...
            <button @click="clearSearch" class="clear-search-button">Показать все фильмы</button>
        </div>
        ...
        <div v-if="!loading && !error && filmworks.length === 0" class="empty-state">
...
<script setup lang="ts">
    ...
    if (isSearchMode.value && searchQuery.value.trim()) {
        const result = await searchFilmworks(searchQuery.value, currentPage.value, pageSize.value)
        filmworks.value = Array.isArray(result) ? result.flat() : []
    } else {
        const result = await getFilmworks(sortBy.value, currentPage.value, pageSize.value)
        filmworks.value = Array.isArray(result) ? result.flat() : []
    }
    }
