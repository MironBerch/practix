<template>
    <div class="filmwork-detail" v-if="filmwork">
        ...
        <div class="poster-section">
            ...
            <div class="rating-section">
                ...
                Средний рейтинг: {{ filmworkRating.average_rating?.toFixed(1) || 'Нет оценок' }}
                </div>
        ...
        <div v-if="showReviewForm" class="review-form">
            ...
            <button @click="submitReview" class="submit-review-btn" :disabled="reviewLoading">
                <span v-if="reviewLoading">Публикация...</span>
                <span v-else>Опубликовать</span>
            </button>
            <button @click="cancelReview" class="cancel-review-btn" :disabled="reviewLoading">Отмена</button>
            </div>
        </div>
        ...
        <div v-if="!reviewsLoading && reviews.length > 0" class="reviews-list">
            ...
            <div class="review-header">
                <span class="review-author">Пользователь #{{ review.author_id.slice(-6) }}</span>
            ...
            <div class="review-rating">
                ...
                <span class="dislikes">👎 {{ reviewRatings[review.id].dislikes }}</span>
                </div>
                
                <div v-if="isAuthenticated" class="rating-actions">
                <button 
                    @click="rateReview(review.id, 1)"
                    :class="['rate-btn', 'like-btn', { 'active': userReviewRatings[review.id] === 1 }]"
                ...
                <button 
                    ...
                    :class="['rate-btn', 'dislike-btn', { 'active': userReviewRatings[review.id] === 10 }]"
                    :disabled="reviewRatingLoading[review.id]"
                >
                    <span v-if="reviewRatingLoading[review.id]">...</span>
                    <span v-else>👎</span>
    ...
    <div v-else-if="error" class="error-state">
        <p>Ошибка: {{ error }}</p>
        <button @click="loadFilmworkData" class="retry-button">Попробовать снова</button>
    </div>
</template>
...
<script setup lang="ts">
    ...
    try {
        ...
        for (const review of reviews.value) {
            const rating = await getFilmworkReviewRating(filmworkUuid.value, review.id)
            if (rating) {
    ...
    try {
        ...
        isBookmarked.value = bookmarks.some((bookmark: any) => 
            bookmark.filmwork_id === filmworkUuid.value
        )
        }
    ...
    if (!isAuthenticated.value) {
        showMessage('Для добавления в избранное необходимо авторизоваться', 'error')
        return
    }
    ...
    try {
        ...
        await addFilmworkReviewRating(filmworkUuid.value, token, reviewScore, reviewId)
