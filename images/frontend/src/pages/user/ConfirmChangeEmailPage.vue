<template>
  <div class="confirm-change-email-page">
    ...
    <form v-else @submit.prevent="handleSubmit" class="confirmation-form">
      ...
      <button type="submit" :disabled="loading || !isCodeValid" class="submit-button">
    ...
    setup() {
      ...
      const handleSubmit = async () => {
        ...
        try {
          ...
          errorMessage.value = error.value || 'Неверный код подтверждения. Попробуйте ещё раз.';
        }
      };
      ...
      const resendCode = async () => {
        ...
        try {
          ...
          if (response) {
            successMessage.value = 'Новый код подтверждения отправлен на ваш email!';
            startCooldown();
          }
        } catch (err) {
          errorMessage.value = error.value || 'Ошибка при отправке кода. Попробуйте позже.';
        } finally {
          resendLoading.value = false;
        }
      };
