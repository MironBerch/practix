...
if TYPE_CHECKING:
    SQL = str
    from schemas import SQLiteSchema
...
class SQLiteDataExtraction:
    ...
    def extract_data(self) -> dict[str, list[SQLiteSchema]]:
        cursor = self.sqlite_connection.cursor()
        cursor.execute("""SELECT * FROM film_work;""")
        film_works = [
            FilmworkSQLite(**film_work) for film_work in cursor.fetchall()
        ]
        cursor.execute("""SELECT * FROM genre;""")
        genres = [
            GenreSQLite(**genre) for genre in cursor.fetchall()
        ]
        cursor.execute("""SELECT * FROM person;""")
        persons = [
            PersonSQLite(**person) for person in cursor.fetchall()
        ]
        cursor.execute("""SELECT * FROM genre_film_work;""")
        genre_film_work = [
            GenreFilmworkSQLite(**genre_film_work) for genre_film_work in cursor.fetchall()
        ]
        cursor.execute("""SELECT * FROM person_film_work;""")
        person_film_works = [
            PersonFilmworkSQLite(**person_film_work) for person_film_work in cursor.fetchall()
        ]
        return {
...
class PostgresDataLoader:
    ...
    def load_data(self, data: dict[str, list[SQLiteSchema]]):
...
def load_from_sqlite(
    ...
    logging.info('Начато извлечение данных из SQLite и загрузка в PostgreSQL')
