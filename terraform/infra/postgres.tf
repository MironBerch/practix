...
# notifications postgres cluster
resource "yandex_mdb_postgresql_cluster" "notifications_db_cluster" {
  name        = "notifications-db-cluster"
  environment = "PRODUCTION"
  network_id  = yandex_vpc_network.network.id
...
resource "yandex_mdb_postgresql_database" "notifications_db" {
  cluster_id = yandex_mdb_postgresql_cluster.notifications_db_cluster.id
  name       = "notifications-db"
  owner      = yandex_mdb_postgresql_user.notifications_db_user.name
  lc_collate = "en_US.UTF-8"
  lc_type    = "en_US.UTF-8"
...
# notifications postgres user
resource "yandex_mdb_postgresql_user" "notifications_db_user" {
  cluster_id = yandex_mdb_postgresql_cluster.notifications_db_cluster.id
  name       = "pguser"
  password   = "pgpassword"
}
...
output "notifications_db_cluster" {
  value       = yandex_mdb_postgresql_cluster.notifications_db_cluster
}

output "notifications_db" {
  value       = yandex_mdb_postgresql_database.notifications_db
}
